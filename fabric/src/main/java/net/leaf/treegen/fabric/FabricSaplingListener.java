 ...
 * {@code /leaftree give}. This mirrors the NeoForge / Paper platforms: the stack carries a species
 * id in its custom-data component (see {@link FabricSaplingItem}), which we read to resolve the
 * species, validate the location, pick a template variant and place the structure.
 */
final class FabricSaplingListener {
    ...
    FabricSaplingListener(Supplier<TreeGenConfig> configRef,
    ...
    /** Registers this listener on the Fabric {@link UseBlockCallback} event. */
    void register() {
    ...
    private InteractionResult onUseBlock(Player player, Level level, InteractionHand hand, BlockHitResult hitResult) {
        ...
        String speciesId = FabricSaplingItem.speciesId(inHand);
     ...
     * structures per {@link FabricPlatform#getWorldNames()} (the save's level-name, e.g.
